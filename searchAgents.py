...
class BehaviourTreeSafeAgent(Agent):
    def __init__(self, genome=None):
        ...
        self.encode = ["SEL", "CheckCapsules.n","aStar.n"]
...
class AStarAgent(Agent):
    ...
    def getAction(self, state):
        ...
        vals = search.aStarSearchAvoid(problem, state, AvoidGhosts)
        return vals[0]
...
def AvoidGhosts(position, state, plan):
    ...
    for i in range(len(xy2)):
        ...
        closestGh = search.breadthFirstSearch(ghProblem)
        ...
        vals = search.aStarSearch(problem,manhattanHeuristic)
        vals2 = search.aStarSearch(problem2, manhattanHeuristic)
...
class GoWestAgent(Agent):
    ...
    def getAction(self, state):
        ...
        ag = ClosestDotSearchAgent()
        return ag.findPathToClosestDot(state)[0]
    ...
    def getAction(self, state):
        ...
        for i in range(self.generations):
            ...
            if(offspringFitness > fitness):
                ...
                print('Chosen: ', offspringFitness)
            else:
                print('Not Chosen: ', offspringFitness)
    ...
    def getFitness(self,plan,state):
        ...
        for actionIndex in range(len(plan)):
            ...
            if action in  nextState.getLegalPacmanActions():
                ...
                if nextState.isWin():
                    return fitness + self.getdiscountedChange(nextState, prevScore, actionIndex)
...
class GAAgent(Agent):
    def __init__(self, genome=None):
        ...
        print "HERE I AM at Genome", self.genome
    ...
    def mutate(self):
        ...
        if(Random_Number>0.5 and Random_Number<0.75):
            self.genome.append(self.legal_leaf[np.random.randint(len(self.legal_leaf))])
        elif(Random_Number>0.75):
            self.genome[1:np.random.randint(len(self.genome))] = self.legal_leaf[np.random.randint(len(self.legal_leaf))]
        return
...
class BehaviourTreeAgroAgent(Agent):
    def __init__(self, genome=None):
        self.legal_composit = ["SEQ", "SEL"]
        ...
        while fringe:
            ...
            if not node in visited:
                ...
                if problem.isGoalState(node):
                    print("giving back actions", actions)
                    return actions
...
class positionSearchP(search.SearchProblem):
    """
    A search problem defines the state space, start state, goal test, successor
    function and cost function.  This search problem can be used to find paths
    to a particular point on the pacman board.
...
class cornerSearchP(search.SearchProblem):
    """
    A search problem defines the state space, start state, goal test, successor
    function and cost function.  This search problem can be used to find paths
    to a particular point on the pacman board.
    ...
    def __init__(self, gameState, costFn = lambda x: 1, goal=(1,1), start=None, initial = (1,1),warn=True, visualize=True):
        """
        Stores the start and goal.
...
class GhostSearchProblem(search.SearchProblem):
    """
    A search problem defines the state space, start state, goal test, successor
    function and cost function.  This search problem can be used to find paths
    to a particular point on the pacman board.
