...
class CompAgent(Agent):
    def __init__(self):
        ...
        '''
        self.encode = ["SEL",
                       ["SEQ", "GhostsCorners", "CheckCapsules"],
    ...
    def __call__(self, state):
        ...
        if not self.firstEaten:
            ...
            for capsule in capsules:
                problem = positionSearchP(state, goal=capsule, visualize=False)
                vals.append(aStarSearchAvoid(problem,state, AvoidGhosts))
            ...
            for index in range(len(gpos)):
                if timer[index] > 10:
                    ...
                    if (x < right/2+offsetx and x > right/2-offsetx) and (y < top/2+offsety and y > top/2-offsety):
                        continue
                    problem = positionSearchP(state, goal=newpos, visualize=False)
                    tmp_val = aStarSearchAvoid(problem,state, AvoidGhosts)
                    vals.append(tmp_val)
